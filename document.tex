...
\title{Distributed Eye Tracking for Conveying Population Gaze in Mobile Telepresence Scenarios} %Fill this in
...
\abstract{
Mobile telepresence technology allows one or several users to visit a distant geographical space
using a mobile robot. These robots can carry a 360$^\circ$ panoramic camera that provides an immersive viewing
environment to the remote users. One limitation of this technology is that for humans around the robot or elsewhere, 
is difficult to gather a sense of where the remote users are paying attention to. We proposed the usage 
of a distributed network of low cost eye trackers, to monitor the gaze behavior and field of view within the panoramic 
video of several users using a telepresence system and transmit that data to the robot that then superimposes the
aggregated gaze behavior of the remote users in a condensed 2D representation of the panoramic image. We use as illustrative 
application a robot being employed to visit a museum virtually by several high school students in remote locations. A
human  museum guide, directs the robot around the museum while explaining and interacting with the remote students
through the visual display on the robot. By focusing on the display with a 2 dimensional representation of the robot's surroundings, 
the museum tour guide can obtain feedback in terms of where in the room the remote students are paying attention to.
